...
- Script hotstring kind: write a raw AutoHotkey v2 body for power users, emitted verbatim inside a brace block. Guarded by brace-balance and no-directive-line validation, and a persistent warning in the edit dialog, grid, and mobile list — AHKFlow never runs generated scripts, so a mistake here only breaks the downloaded `.ahk` file. Supported in the edit dialog (monospace editor, "Use Script?" suggestion for Text replacements that look like code), grid, mobile list, and `ahkflow hotstring list`.
