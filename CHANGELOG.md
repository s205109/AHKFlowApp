...
- Clipboard delivery for Text hotstrings, with Auto, Typed, and Clipboard choices in the web UI and `--delivery auto|type|clipboard` plus UTF-8 `--replacement-file` support in the CLI.
...
- Existing Text hotstrings with replacements of 200 characters or more now paste through the clipboard by default; shorter replacements continue to be typed. Choose Typed delivery to retain the previous behavior for a long replacement.
