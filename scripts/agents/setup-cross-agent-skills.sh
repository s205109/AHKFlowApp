...
# .agents/plugins/ may exist without one. There are no parking directories: retiring
# a skill means deleting it.
...
is_active_skill() {
    local name="$1"
    # plugins is the only .agents/ subdirectory that is not a skill. Retiring a skill
    # means deleting it, so there are no parking directories for disabled or
    # reference-only content. skills is not on this list: the block above already
    # removed .agents/skills/ or stopped the run. tests/SkillLayout.Tests.ps1 enforces
    # the same list. Keep the two in sync.
    case "$name" in
        plugins)
            return 1
            ;;
