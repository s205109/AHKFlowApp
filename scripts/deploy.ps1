...
APP_SERVICE_HOSTNAME=$AppHostname
...
DEPLOYER_UAMI_CLIENT_ID=$DeployerUamiClientId
RUNTIME_UAMI_NAME=$RuntimeUamiName
SQL_ADMIN_GROUP=$SqlAdminGroupName
...
$envContent | Set-Content -Path (Join-Path $PSScriptRoot ".env.$Environment") -Encoding UTF8
...
Write-Host "==========================================================" -ForegroundColor Green
Write-Host ""
Write-Host "  API (App Service)  : https://$AppHostname/health"
Write-Host "  Frontend (SWA)     : https://$SwaHostname"
Write-Host "  Resource Group     : $ResourceGroup"
Write-Host ""
Write-Host "  Next Steps:" -ForegroundColor Cyan
Write-Host "  1. Push to 'main' to trigger deploy-api.yml + deploy-frontend.yml"
Write-Host "  2. The first push will publish the container image to GHCR."
Write-Host ""
Write-Warn "IMPORTANT: GHCR packages are private by default."
Write-Host "  After the first CI push, make the container image public:"
Write-Host "  https://github.com/orgs/${Owner}/packages" -ForegroundColor Yellow
Write-Host ""
...
Invoke-Az webapp config connection-string set `
    ...
    --settings "DefaultConnection=Server=tcp:${SqlServerFqdn},1433;Database=${SqlDatabaseName};Authentication=Active Directory Default;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;" | Out-Null
Write-Success "App Service connection string configured"
...
Write-Success "App Service CORS configured (https://$SwaHostname)"

# ---------------------------------------------------------------------------
# Phase 8: Save config & Print Summary
# ---------------------------------------------------------------------------

Write-Step "Phase 8: Saving configuration and printing summary..."

$EnvFile = Join-Path $PSScriptRoot ".env.${Environment}"
@"
# AHKFlowApp deployment config — generated by deploy.ps1
# DO NOT COMMIT — this file is gitignored
ENVIRONMENT=$Environment
RESOURCE_GROUP=$ResourceGroup
BASE_NAME=$BaseName
...
Write-Success "Configuration saved to: scripts/.env.$Environment"
...
Write-Host "==========================================================" -ForegroundColor Green
Write-Host ""
Write-Host "  API URL    : https://$AppServiceHostname/health"
Write-Host "  Frontend   : https://$SwaHostname"
Write-Host ""
Write-Host "  Next steps:" -ForegroundColor White
Write-Host "  1. Push (or merge a PR) to main to trigger GitHub Actions deploy"
Write-Host ""
Write-Host "  IMPORTANT — GHCR package visibility:" -ForegroundColor Yellow
Write-Host "  After the first deploy-api.yml run, the container image in GHCR will be private."
Write-Host "  The App Service cannot pull a private GHCR image without credentials."
Write-Host "  Make the package public:"
...
Write-Host ""
Write-Host "  To update to a new release   : .\scripts\update.ps1 -Environment $Environment"
Write-Host "  To tear down the environment : .\scripts\teardown.ps1 -Environment $Environment"
Write-Host ""
