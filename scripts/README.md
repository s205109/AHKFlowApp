...
| `cleanup-merged-worktrees.ps1` | Detects worktrees merged into the base and removes the clean ones. The base is the fetched remote-tracking branch (`origin/main`); an explicit `-MainRef` is used as given and skips the fetch. A failed fetch is reported and forces report-only for that run: the cached ref can hold a merge the remote has dropped, and removal cannot be undone. Opt-in via `git config --local ahkflow.worktreeCleanup true` (the recommended set-once switch); unset asks once on an interactive create; `AHKFLOW_WORKTREE_CLEANUP` (hook context only) and `-Cleanup` are per-run overrides. Invoked by `new-worktree.ps1` before it creates a worktree. A branch nobody has committed on is never swept, whatever the setting says: a brand-new worktree points at the base's tip, so `--merged` lists it. Removal needs three signals to agree: the ref log holds a subject for an operation that creates a commit, one of those SHAs (or a `rebase (finish)` SHA) is a non-first parent of a merge commit reachable from the base, and removing the branch would strand no commit that a `git reset` dropped. Reading the whole ref-log history, not just the current tip, keeps a merged worktree sweepable after it catches up with `git merge --ff-only main`. Whatever it cannot establish is kept. It also reports the other half-finished removal — a branch merged into the base whose worktree is already pruned — on a `cleanup: leftover branch` line, decided by the same three signals. That report deletes nothing; it names the `git branch -d` to run. |
