...
- Confirm the base before branching, and state it. Pass `-BaseRef <branch>` to `scripts/new-worktree.ps1` for work that builds on an unmerged branch. Every worktree is created by that script; the session then moves into it, and in Claude Code that move uses the native worktree tool. One reason for the split is that the tool cannot pass a base ref — see [workflow.md#stage-1-pickup](docs/development/workflow.md#stage-1-pickup).
