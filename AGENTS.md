...
- Add a folder under `backlog/` in one place only: `$script:BacklogItemSubfolder` in `scripts/backlog.common.ps1`. Six path patterns in three other scripts read git rather than the working tree and carry their own copy of the list, and `tests/BacklogFolderParity.Tests.ps1` fails when a copy stops matching. A folder no copy knows about still holds items, and their numbers stop counting as taken — see [workflow.md#stage-1-pickup](docs/development/workflow.md#stage-1-pickup).
