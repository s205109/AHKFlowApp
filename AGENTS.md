...
- Move the session into the worktree at Pickup, with the native `EnterWorktree` tool and the `path` the script printed. Creating a worktree and staying in the main checkout loses the location on resume, and leaves this repository's worktree write guard switched off — see [workflow.md#stage-1-pickup](docs/development/workflow.md#stage-1-pickup).
