...
- Open every plan with a `## Split` section: the estimate in work sessions, the task at which the user story closes, and a split verdict. A plan at the split trigger must evaluate a split in writing, and `scripts/check-plan-split-record.ps1` refuses the push otherwise. Rules: [workflow.md#the-split-record](docs/development/workflow.md#the-split-record) — see [workflow.md#stage-3-plan](docs/development/workflow.md#stage-3-plan).
