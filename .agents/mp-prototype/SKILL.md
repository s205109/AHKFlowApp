...
4. **Skip the polish.** No unit tests, no error handling beyond what makes the prototype _runnable_, no abstractions. The point is to learn something fast.
   ...
   - **Logic prototype:** serve the folder over HTTP first, then open the page with the `playwright-cli` skill. `playwright-cli` refuses a `file:` URL — see **In this repo** in [LOGIC.md](LOGIC.md) for the two commands. Click through the free-play buttons and each guided walkthrough, and attach a screenshot of the resulting state after each.
...
6. **Capture it when done — capture first, fold second.** Order matters, because folding deletes things. Start by capturing the prototype itself as a **primary source**: commit the complete prototype to a throwaway branch, out of main, and leave a context pointer to that branch on the implementation issue. Capture the answer too — the verdict and the question it settled — in the issue or a commit. Only then fold the validated decision into the real code. Folding in the other order destroys the losing variants before anything preserved them. Main keeps only the validated decision.

   In this repo, do both steps from a managed worktree. Create it with `scripts/new-worktree.ps1`, or with the native `EnterWorktree` tool, which runs the same script. **Git Workflow** in `AGENTS.md` explains why: an agent `git commit` in the main checkout is always refused, and a worktree session may not write to a path under main either. So neither the throwaway branch nor the fold can happen from main. Write and commit both in the worktree, then open a pull request for the fold.
