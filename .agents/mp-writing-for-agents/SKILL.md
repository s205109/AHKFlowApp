...
Three repo rules bind on top of the reference above. They do not replace it.

- **Plain English wins over compression.** `AGENTS.md` **Plain English** applies to skill files, and it asks for the opposite trade to pruning: when short and easy-to-read conflict, use more words. Prune load-bearing nothing — no-ops, duplication, sediment — but do not compress a live sentence into something the reader must decode twice.
- **Skill descriptions are capped at 140 characters.** `scripts/agents/setup-cross-agent-skills.ps1` warns above that. The cap is the context-load budget made concrete, so treat it as the pointer-writing exercise, not a truncation job.
- **The layout is generated, and `.claude/skills/README.md` owns the how.** Read it before adding, retiring, or moving a skill. It covers the three projected locations and the setup script you re-run after any `.agents/` change.

An adapted upstream skill carries a header comment naming its pinned commit. Keep the upstream prose intact and append an "In this repo" section like this one, so a later selective merge still lines up. `.agents/ATTRIBUTION.md` records what was adapted and why.
