...
public sealed class PreferencesController(
    IUseCase<GetUserPreferenceQuery, Result<UserPreferenceDto>> getUserPreference,
    IUseCase<UpdateUserPreferenceCommand, Result<UserPreferenceDto>> updateUserPreference) : ControllerBase
{
    ...
    public async Task<ActionResult<UserPreferenceDto>> Get(CancellationToken ct) =>
        (await getUserPreference.ExecuteAsync(new GetUserPreferenceQuery(), ct)).ToProblemActionResult(this);
    ...
    public async Task<ActionResult<UserPreferenceDto>> Update([FromBody] UpdateUserPreferenceDto dto, CancellationToken ct) =>
        (await updateUserPreference.ExecuteAsync(new UpdateUserPreferenceCommand(dto), ct)).ToProblemActionResult(this);
}
