...
- [ ] **Step 1:** Add `string? Description` and `Guid[] CategoryIds` to the CLI `HotstringDto` record (positions matching backend: Description after Replacement, CategoryIds last, default `[]` via a constructor default is NOT possible on positional records — make it `Guid[]? CategoryIds = null` and normalize with `?? []` at use sites, or add it as required positional and fix construction sites). Add `string? Description = null` to the CLI `CreateHotstringDto`.
