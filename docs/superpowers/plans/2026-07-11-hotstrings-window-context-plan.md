...
- **Import duplicate detection stays trigger-only *within the global scope*** — imported rows are always global (no context), so they only ever collide with existing **global** rows under the new composite index. Filter both existing-trigger queries to global rows so a contexted `btw` never blocks importing a global `btw`:
  - `PreviewHotstringImportCommand.cs` l.45–48: add `&& h.ContextMatchType == null && h.ContextValue == null` to the existing-trigger query.
  - `ImportHotstringsCommand.cs` l.94–96: same filter on the duplicate-key retry re-query, so a context-only survivor is never discarded when a *different* row causes a genuine global collision.
  - Duplicate reason stays global-scoped (imports never create context) — confirm `HotstringImportClassifier.DuplicateReason` text still reads correctly for a global collision.
...
- Import regressions: `Preview_TriggerExistsOnlyAsContexted_MarksGlobalRowReady`, `Import_TriggerExistsOnlyAsContexted_ImportsGlobalRow`, `Import_RetryWithGlobalCollision_KeepsContextOnlySurvivors` (a genuine global collision on one trigger must not drop a different import row whose trigger only exists contexted).
...
- `Validation/HotstringEditModel.cs`: fields + `[MaxLength(200)]`; thread `FromDto`/`Clone`/`ToCreateDto`/`ToUpdateDto`; `IsInlineEditable => Kind == HotstringKind.Text && ContextMatchType is null`; `ContextSummary` helper (`exe:` / `class:` / `title:` prefix) reused by grid tooltip + mobile. **Context Value is locally required *while context is enabled*** — DataAnnotations can't express the conditional, so gate it in the dialog (field-level `Required` param bound to "context on", mirroring the DateTime/Replacement pattern noted at l.14–15), not just `[MaxLength]`. Empty value while context on must block save client-side, before the API.
- `HotstringEditDialog.razor`: new "Window context" section (subtitle pattern like "Trigger options" l.158): `MudSwitch` **Only in specific windows** (on → default `Executable`; off → clear both); when on: `MudSelect<WindowMatchType>` **Match by** (Program / Window class / Title contains) + `MudTextField` **Value** (`Required` when context on) with per-type placeholder (`notepad.exe` / `Chrome_WidgetWin_1` / `- Visual Studio`); `data-test` attrs. `BuildPreviewRequest()` (l.469) gains both fields so preview re-fires on context edits. **Route `Input.ContextValue` preview errors inline**: extend `ApplyPreviewResult`'s field mapping (l.561–575) — add a `_previewContextValueError` field mapped from the `ContextValue` key, bound onto the Value field's `ErrorText`, so server context validation shows next to the field instead of falling into the generic panel.
