...
- **Clean slate.** Commit `46ce04a "Removed skills"` deleted the earlier `mp-handoff`/`grilling` experiments. The untracked `.claude/skills/grilling/grilling/` folder (manual re-add for the grilling session) is removed automatically by the sync script (non-symlink entries are pruned) — superseded by `mp-grilling`.
...
- The setup skill only configures triage labels when it detects the triage skill installed → `mp-triage` must be vendored **before** the setup runs.
...
| setup-matt-pocock-skills | **Copy as `mp-setup-matt-pocock-skills`, run full setup interactively (Wave 2), keep afterwards** with `disable-model-invocation: true` |
...
|---|---|---|---|
| `grilling` | Reusable relentless one-question-at-a-time interview loop | **Copy** → `mp-grilling` | Foundation for grill-me, grill-with-docs, triage. Complements `superpowers:brainstorming` — grilling interrogates, brainstorming designs. |
...
| `domain-modeling` | Maintains domain glossary (`CONTEXT.md`) + sparse ADRs; challenges terminology, tests edge cases against code | **Copy as-is (whole folder)** → `mp-domain-modeling` | Includes `CONTEXT-FORMAT.md`/`ADR-FORMAT.md`. High value: AGENTS.md Domain Terms already drift. |
...
| `setup-matt-pocock-skills` | One-time repo config scaffolder (tracker, labels, domain docs) | **Copy** → `mp-setup-matt-pocock-skills` (whole folder incl. seed templates); run in Wave 2; keep | Already `disable-model-invocation: true` upstream — retain. |
...
1. Copy into `.agents/`: `mp-grilling`, `mp-grill-me`, `mp-domain-modeling`, `mp-grill-with-docs`, `mp-handoff`, `mp-triage`, `mp-setup-matt-pocock-skills` (each whole folder); rewrite internal cross-refs to `mp-` names; `name:` frontmatter matches folder.
2. AHKFlow-owned-adaptation header comment in each SKILL.md (upstream: `mattpocock/skills`, update-policy: manual-selective-merge).
3. Add `mattpocock/skills` (MIT) section to `.agents/ATTRIBUTION.md`.
4. Run `setup-cross-agent-skills.ps1` (also prunes the stray untracked `.claude/skills/grilling/grilling/`); verify symlinks/hard links; commit, PR.
...
1. `gh label create` ×5 (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`) in `s205109/AHKFlowApp`.
...
- Sync script clean; `.claude/skills/` shows all seven `mp-*` symlinks; Codex plugin hard links present; stray `grilling/grilling` gone.
...
- Wave 4: `gh label list` shows five labels; test triage run produces correct labels/comment (with AI-generated notice).
