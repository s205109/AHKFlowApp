...
- 2026-06-23 hotstring mobile CRUD narrowing: replaced the hotstring phone create/edit/delete E2E flow with a focused phone FAB/dialog smoke after confirming desktop hotstring browser CRUD, bUnit hotstring page/dialog/mobile-list coverage, and API/Application CRUD tests cover the removed behavior at cheaper layers. The hotkey phone CRUD flow remained the last candidate for bounded E2E narrowing because its CRUD semantics already had cheaper-layer coverage. E2E-only measurement passed 6 tests at 44.2s wall / 27.9s summed test duration / 16.4s unattributed setup. A fresh full measurement passed all projects with E2E at 45.3s, Infrastructure 24.7s, CLI 22.2s, API 21.9s, Application 19.8s, UI 7.0s, and Domain 1.9s. Full coverage still passed at 90.2% line / 59.7% branch. Next sequencing at that point: inspect the hotkey phone CRUD path before returning to SQL startup reuse.
- 2026-06-23 hotkey mobile CRUD narrowing: fresh pre-change measurement passed all projects with E2E still largest at 47.3s wall / 30.5s summed test duration; `HotkeysMobileFlowTests.CreateEditDelete_OnPhoneViewport_UsesFabAndFullScreenDialog` was the slowest single test at 11.5s. Replaced that full browser CRUD flow with `AddFab_OnPhoneViewport_OpensHotkeyDialog` after confirming `HotkeyEditDialogTests.SaveInCreateMode_CallsCreateAsync`, `HotkeyEditDialogTests.SaveInEditMode_CallsUpdateAsync`, `HotkeysPageTests.Page_SaveDraftRow_CallsCreate`, `HotkeysPageTests.Page_EditExistingRow_CallsUpdate`, `HotkeysEndpointsTests.Delete_ThenGet_Returns404`, and the Application hotkey create/update/delete handler tests cover the removed CRUD behavior at cheaper layers. Targeted hotkey-mobile E2E passed, then E2E-only measurement passed 6 tests at 29.9s wall / 16.1s summed test duration / 13.7s unattributed setup; `HotkeysMobileFlowTests` dropped from 18.2s to 7.8s. Next sequencing: run a fresh full `measure-tests.ps1` after this commit; if E2E stays bounded, move back to cross-project SQL startup reuse for API/Infrastructure/CLI/Application.
- 2026-06-23 fresh post-hotkey-mobile-narrowing measurement: `measure-tests.ps1` passed all projects; E2E stayed bounded but remained the largest single project at 36.2s wall / 19.3s summed test duration / 16.9s unattributed setup, followed by CLI 24.1s, API 21.0s, Infrastructure 20.2s, Application 19.1s, UI 6.5s, and Domain 2.8s. The largest individual setup entry was still `StackFixture.InitializeAsync` at 13.7s, but the four separate `SqlContainerFixture.StartAsync` entries in CLI, Application, Infrastructure, and API totaled about 40.9s. Next sequencing: stop E2E pruning for now and implement bounded SQL startup reuse across the SQL-backed non-E2E suites, starting with the common `SqlContainerFixture` consumers and preserving automatic Testcontainers cleanup.
- 2026-06-27 bounded shared SQL startup reuse: `scripts/test-sql-container.common.ps1` now starts one disposable Docker SQL Server container for local Integration/Coverage/measurement runs and passes `AHKFLOW_TEST_SQL_CONNECTION_STRING` to SQL-backed test processes. `SqlContainerFixture` derives deterministic per-test-assembly databases from that server connection and does not dispose the runner-owned container; the normal per-project Testcontainers path remains the fallback for direct `dotnet test` runs. A fresh full `measure-tests.ps1` passed all projects with one shared SQL startup at 9.5s instead of four separate `SqlContainerFixture.StartAsync` entries; wall-clock timings were E2E 36.0s, Infrastructure 9.9s, API 9.9s, CLI 8.4s, Application 8.1s, UI 6.3s, and Domain 1.5s. `StackFixture.InitializeAsync` is again the largest setup entry at 13.2s, followed by the script-owned shared SQL startup and Application `MigratedDbFixture.MigrateAsync` entries around 3.1-3.8s. Next sequencing: keep the shared SQL path and focus any further speed work on the remaining measured hotspots rather than adding broader container reuse.
- 2026-06-27 E2E shared SQL startup reuse: `ApiFactory` now honors `AHKFLOW_TEST_SQL_CONNECTION_STRING`, derives a deterministic `AHKFlowApp.E2E.Tests` database, and skips owning a Testcontainers SQL container when the runner already started one. `test-fast.ps1 -Mode E2E` and E2E-only `measure-tests.ps1` now use the same script-owned disposable SQL server path as Integration/Coverage. A targeted E2E measurement passed 7 tests (6 browser flows plus the fixture guard) at 19.8s wall with `StackFixture.InitializeAsync` down to 3.2s; the fresh full measurement passed all projects with E2E at 20.3s wall, shared SQL startup at 8.4s, and `StackFixture.InitializeAsync` at 3.0s. The remaining slow setup entries are now Application `MigratedDbFixture.MigrateAsync` entries around 2.9-3.6s, while the slowest individual tests are mostly browser/CLI behavior rather than container startup. Next sequencing: stop startup-reuse work here unless a future measurement shows migration cloning would beat the added complexity.
...
- Share SQL startup in local runners:
  - Start one script-owned disposable SQL Server container for `test-fast.ps1 -Mode Integration`, `test-fast.ps1 -Mode E2E`, `run-coverage.ps1`, and SQL-backed `measure-tests.ps1` selections.
  - Pass the base server connection through `AHKFLOW_TEST_SQL_CONNECTION_STRING`, then let `SqlContainerFixture` derive deterministic per-test-assembly databases.
  - Restore the previous environment value and remove the Docker container in `finally`; do not use Testcontainers reusable-container mode.
...
- Verify `scripts/test-fast.ps1 -Mode Integration` runs SQL/API-backed tests, including whole-project `Infrastructure.Tests`, through one script-owned SQL container.
- Verify `scripts/test-fast.ps1 -Mode E2E` runs the 6 Playwright browser tests plus the fixture guard through one script-owned SQL container.
