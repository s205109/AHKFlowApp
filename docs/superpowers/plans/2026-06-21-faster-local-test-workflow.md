...
- 2026-06-22 full post-conflict-pruning measurement: `measure-tests.ps1` passed all projects; E2E remained largest at 75.1s wall / 56.1s summed test duration / 19.0s unattributed setup, followed by API 56.0s, Application 28.6s, Infrastructure 28.4s, CLI 25.0s, UI 10.0s, and Domain 1.9s. The slowest classes were still E2E (`HotkeysMobileFlowTests` 27.3s, `HotstringsMobileFlowTests` 17.9s, `HotstringsCrudFlowTests` 10.9s), but API now has the largest single setup cost (`SqlContainerFixture.StartAsync` 19.8s). Next sequencing: do one more E2E overlap audit for `HotstringsCrudFlowTests` and remaining mobile CRUD/bulk coverage; if those flows must stay, move to API/Infrastructure SQL fixture setup reuse next.
...
- Verify `scripts/test-fast.ps1 -Mode E2E` runs the 7 Playwright tests.
