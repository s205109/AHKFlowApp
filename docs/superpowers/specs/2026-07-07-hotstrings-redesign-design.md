...
11. **D11 — Script reuses `Replacement`**; no new domain/DTO/snapshot columns, no migration (Phase 5).
12. **D12 — Brace-balance check is naive `{`/`}` character counting** (no string-literal awareness) — consistent with D8's boundary. Known limitation: a `{` inside a quoted string (e.g. `SendText "{"`) counts toward the balance, false-positive-rejecting an otherwise valid script. Documented, not fixed. A string/comment-aware lexical scanner was considered and rejected: it crosses D8's boundary and introduces its own edge cases (escaped quotes, continuation sections).
13. **D13 — CLI table Replacement-summary switch gets a Script first-line branch only.** Macro rows keep the raw replacement as-is (Phase 3 decision) — no Macro token-summary in Phase 5. Reverting that would require the CLI to reference the Application layer or duplicate `MacroTokenParser`; deferred to a future CLI-display follow-up.
14. **D14 — Grid + mobile kind-filter dropdowns gain Macro and Script items** (pre-existing gap: only Text/DateTime were listed), fixed alongside the Script UI work (Phase 5).
15. **D15 — Phase 5 docs note = CHANGELOG entry + new `docs/cli/hotstrings.md` per-kind CLI reference doc** (first per-kind CLI doc; prior phases only did CHANGELOG entries).
