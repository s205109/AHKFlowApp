...
**Raw is the sole verbatim path, and it is not sandboxed.** Its `Body` is emitted exactly as stored — no wrapper, no escaping, so a block body carries its own braces. Validation checks shape only: non-empty, balanced `{`/`}`, no line starting with a `#` directive, no stray control characters. AutoHotkey also parses the whole file at load, so a syntax error in one Raw body aborts the **entire** Profile script rather than just its own binding, and the brace-balance check has no awareness of string literals or comments, so a body can close its block early and have its remainder parsed at top level. ADR-0002 took the same position for Raw hotstrings — Raw exists precisely for what structured fields cannot express, so its text is kept as the source of truth rather than decomposed — and the naive brace count is the trade-off recorded under *Known limitations* in `docs/development/ahk-v2-syntax.md`: a string- and comment-aware scanner would drift toward being a script IDE. Raw must therefore be presented as unchecked AutoHotkey wherever it is offered.

> **Amendment, 2026-08-03 (backlog 037):** the example in the previous paragraph — a body of
> `return` followed by a second line `^a::Run("calc")` — no longer passes validation. `HotkeyRules`
> now rejects any Raw body line that opens a new top-level hotkey or hotstring definition
> (`RawHotkeyBodyScanner`), closing exactly the injection this paragraph described. That scanner is
> not the naive brace-balance check: it keeps its own comment- and string-aware depth, specifically
> so a line comment or a block comment cannot hide an injected definition from it. The naive
> brace-balance check itself is unchanged — it still has no awareness of string literals or
> comments, and the trade-off this paragraph records still applies to it. See
> `docs/development/ahk-v2-syntax.md`, "`Raw` is unchecked".
...
A `Send` whose parameters do parse as a valid key token becomes `SendKeys`, and that branch does change: `SendKeys` now carries the auto `$` prefix the pre-W1 emitter never wrote, so `^!v::Send("^v")` regenerates as `$^!v::Send("^v")`. It is a large share of legacy rows — every registry name, alias and `vk`/`sc` code qualifies in braced form (`{Enter}`, `{Esc}`, `{vk1}`), as does any single printable character, optionally with `^ ! + #` modifiers (`LegacyHotkeyFixtures`) — so downloading an untouched profile after the migration does change those users' scripts. The change is intended: without `$` a `SendKeys` binding can retrigger itself.
