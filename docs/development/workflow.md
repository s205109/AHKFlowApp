...
2. **Deferred to the watcher.** The session triggers removal, **updates memory before it
   ...
   branch (`scripts/remove-worktree-local-dev.ps1:1320`, "        $branchDelete = Invoke-GitCapture @('-C', $mainCheckout, 'branch', '-d', '--', $branchName)"). After a clean run there is no
   ...
   (`scripts/remove-worktree-local-dev.ps1:1310`, "    $pruneResult = Invoke-GitCapture @('-C', $mainCheckout, 'worktree', 'prune', '-v')") before it deletes
   the branch, and has a documented outcome that stops in between, logging
   (`scripts/remove-worktree-local-dev.ps1:1395`, "        Write-DiagnosticLog 'Watcher done (worktree removed; branch preserved).'").
...
(`scripts/remove-worktree-local-dev.ps1:1320`, "        $branchDelete = Invoke-GitCapture @('-C', $mainCheckout, 'branch', '-d', '--', $branchName)").
