...
the base (`scripts/worktree-git.common.ps1:722`, "function Test-BranchOwnWorkWasMerged {"). A merge
...
(`scripts/worktree-git.common.ps1:697`, "function Get-WorkAfterMergeProof {"). Push that work, or
...
2. **Deferred to the watcher.** The session triggers removal, **updates memory before it
   ...
   branch (`scripts/remove-worktree-local-dev.ps1:981`, "        $branchDelete = Invoke-GitCapture @('-C', $mainCheckout, 'branch', '-d', '--', $branchName)"). After a clean run there is no
   ...
   (`scripts/remove-worktree-local-dev.ps1:975`, "    Write-GitResult 'worktree prune -v' (Invoke-GitCapture @('-C', $mainCheckout, 'worktree', 'prune', '-v'))") before it deletes
   the branch, and has a documented outcome that stops in between, logging
   (`scripts/remove-worktree-local-dev.ps1:1056`, "        Write-Log 'Watcher done (worktree removed; branch preserved).'").
...
(`scripts/remove-worktree-local-dev.ps1:981`, "        $branchDelete = Invoke-GitCapture @('-C', $mainCheckout, 'branch', '-d', '--', $branchName)").
