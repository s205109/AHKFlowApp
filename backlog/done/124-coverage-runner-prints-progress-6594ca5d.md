...
- [x] `scripts/run-coverage.ps1` prints a progress line before each of its units through
      ...
      (`scripts/run-coverage.ps1:35`, "$PSScriptRoot\progress.common.ps1");
      (`scripts/run-coverage.ps1:71`, "Start-ProgressUnit -Tracker $progress -Name 'restore'");
      (`scripts/run-coverage.ps1:76`, "Start-ProgressUnit -Tracker $progress -Name 'build'");
      (`scripts/run-coverage.ps1:88`, "Start-ProgressUnit -Tracker $progress -Name 'sql container'");
      (`scripts/run-coverage.ps1:102`, "Start-ProgressUnit -Tracker $progress -Name $projectName");
      (`scripts/run-coverage.ps1:141`, "Start-ProgressUnit -Tracker $progress -Name 'report'")
- [x] Its unit list names the restore, the build, each coverage project, and the report step.
      (`scripts/run-coverage.ps1:68`, "$progressUnit = @('restore', 'build', 'sql container') + $expectedProjectName + @('report')").
...
- [x] Its remembered timings live under their own runner key, so they never mix with the keys
      ...
      (`scripts/run-coverage.ps1:69`, "New-ProgressTracker -RunnerKey 'run-coverage'");
      (`scripts/run-coverage.ps1:152`, "Save-ProgressTimings -Tracker $progress -KnownUnit $progressUnit");
      (`tests/CoverageRunnerProgress.Tests.ps1:409`, "Invoke-TestCase 'The coverage store never mixes with a test-fast store' {")
- [x] A coverage run started through `scripts/test-fast.ps1 -Mode Coverage` prints one progress
      sequence, not two nested ones.
      (`tests/CoverageRunnerProgress.Tests.ps1:483`, "Invoke-TestCase 'Coverage through test-fast prints one sequence, not two' {")
- [x] `tests/Progress.Tests.ps1`, or a suite beside it, covers a unit list that mixes fixed
      ...
      (`tests/CoverageRunnerProgress.Tests.ps1:366`, "Invoke-TestCase 'The project part of the unit list is read at run time' {")
...
- The read of the coverage projects moved above the restore, because a tracker needs its whole
  ...
  (`scripts/run-coverage.ps1:59`, "$coverageProject = Get-AhkFlowCoverageProject -RepoRoot $repoRoot").
