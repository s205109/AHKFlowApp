...
(`scripts/pre-push-quick-checks.ps1:49`, "    Write-Step 'Checking citations in the private plans repository'")
and
(`scripts/pre-push-quick-checks.ps1:128`, "    Write-Step 'Checking the plans against the source'").
...
- [x] `git push` completes with the pre-push hook enabled, without `SKIP_PUSH_HOOK=1` and without `--no-verify`
  - Proven on 2026-08-21, and again on 2026-08-22 after review round 1 and after merging that day's `main`. All five hook steps green both times, ending `Pre-push quick checks passed.` The citation step printed `Checking 1: plans/2026-08-21-stale-plans-citations-plan-112.md`, and the new freeze step printed `Shipped plans are frozen.`
