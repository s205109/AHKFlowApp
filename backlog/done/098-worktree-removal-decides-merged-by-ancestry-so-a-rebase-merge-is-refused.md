...
(`scripts/worktree-git.common.ps1:808`, "function Test-BranchOwnWorkWasMerged {"), after a
first filter of `git branch --merged`. That filter is gone now: the shared decision is the only
gate (`scripts/cleanup-merged-worktrees.ps1:119`, "        if (-not (Test-BranchOwnWorkWasMerged -RepoRoot $RepoRoot -Branch $wt.Branch -MainRef $MainRef -MergedPullRequests $MergedPullRequests)) { continue }").
...
merge commit on main (`scripts/worktree-git.common.ps1:544`, "    $parentLines = & git -C $RepoRoot rev-list --min-parents=2 --format='%P' $MainRef 2>$null"),
