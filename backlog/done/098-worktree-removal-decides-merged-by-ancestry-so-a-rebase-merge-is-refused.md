...
(`scripts/remove-worktree-local-dev.ps1:956`, "        if (-not (Test-WorktreeMergedIntoMain -WorktreeFull $worktreeFull -BranchName $branchName -BaseRef $baseRef -MainCheckout $mainCheckoutFromGit)) {").
...
- The Cleanup warning that pointed at this item is gone. Stage 10 now describes the shared rule
  instead (`docs/development/workflow.md:577`, "**Any merge button is fine.** Both scripts decide with one rule: did this branch's own work reach")
