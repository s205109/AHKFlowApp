...
(`scripts/remove-worktree-local-dev.ps1:958`, "        if (-not (Test-WorktreeMergedIntoMain -WorktreeFull $worktreeFull -BranchName $branchName -BaseRef $baseRef -MainCheckout $mainCheckoutFromGit)) {").
The sweep asks a different question: reachability plus a ref-log reading of the branch's own work
(`scripts/worktree-git.common.ps1:1155`, "function Test-BranchOwnWorkWasMerged {"), after a
first filter of `git branch --merged`. That filter is gone now: the shared decision is the only
gate (`scripts/cleanup-merged-worktrees.ps1:146`, "        $mergedVerdict = Get-BranchMergedVerdict -RepoRoot $RepoRoot -Branch $wt.Branch -MainRef $MainRef -MergedPullRequests $MergedPullRequests").
...
merge commit on main (`scripts/worktree-git.common.ps1:736`, "    $parentLines = & git -C $RepoRoot rev-list --min-parents=2 --format='%P' $MainRef 2>$null"),
...
- The Cleanup warning that pointed at this item is gone. Stage 10 now describes the shared rule
  instead (`docs/development/workflow.md:608`, "**Any merge button is fine.** Both scripts decide with one rule: did this branch's own work reach")
