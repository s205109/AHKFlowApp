...
(`scripts/remove-worktree-local-dev.ps1:1226`, "    $pruneResult = Invoke-GitCapture @('-C', $mainCheckout, 'worktree', 'prune', '-v')") first and deletes the
branch (`scripts/remove-worktree-local-dev.ps1:1236`, "        $branchDelete = Invoke-GitCapture @('-C', $mainCheckout, 'branch', '-d', '--', $branchName)") second,
and logs (`scripts/remove-worktree-local-dev.ps1:1311`, "        Write-DiagnosticLog 'Watcher done (worktree removed; branch preserved).'") when
