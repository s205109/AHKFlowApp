...
(`scripts/remove-worktree-local-dev.ps1:1203`, "    $pruneResult = Invoke-GitCapture @('-C', $mainCheckout, 'worktree', 'prune', '-v')") first and deletes the
branch (`scripts/remove-worktree-local-dev.ps1:1213`, "        $branchDelete = Invoke-GitCapture @('-C', $mainCheckout, 'branch', '-d', '--', $branchName)") second,
and logs (`scripts/remove-worktree-local-dev.ps1:1288`, "        Write-DiagnosticLog 'Watcher done (worktree removed; branch preserved).'") when
